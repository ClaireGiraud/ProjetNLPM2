...
mod_TL.fit(x_train, y_train, batch_size=30, epochs=5)
...
print('##################################################################')
print("Test sur notre jeu de données traduites")
...
# on met les données sous forme de liste
news2=own_data['text'].values.tolist()
cat2=own_data['category'].values.tolist()

#on definit le nouveau jeu own_data comme l'échantillon test
x_test2 = vectorizer(np.array([[s] for s in news2])).numpy()
y_test2 = np.array(cat2)


mod_TL.evaluate(x_test2, y_test2)
pred2 = mod_TL.predict(x_test2)

pred2 = pd.DataFrame(data=pred2, columns=["0", "1"])
del pred2['1']

pred_array2 = pred2.to_numpy()

binary_predictions2 = []

for j in pred_array2:
    if j >= 0.5:
...
print('Accuracy on testing set:', accuracy_score(binary_predictions2, y_test2))
print('Precision on testing set:', precision_score(binary_predictions2, y_test2))

matrix_TL2 = confusion_matrix(binary_predictions2, y_test2, normalize='all')
graph (matrix_TL2)
